...
urlpatterns = patterns('',
    ...
    url(r'^(?P<mode_1>[\+-])/(?P<major_interval_1>\d+)/(?P<minor_interval_1>\d+)/(?P<name_1>[a-zA-Z]{1})/(?P<mode_2>[\+-])/(?P<major_interval_2>\d+)/(?P<minor_interval_2>\d+)/(?P<name_2>[a-zA-z]{1})/(?P<x1>\d+)/(?P<y1>\d+)/(?P<x2>\d+)/(?P<y2>\d+)/$', views.GraphPath, name='GraphPath'),
